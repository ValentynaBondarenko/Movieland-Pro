...
class ParallelEnrichmentServiceITest extends AbstractITest {
    ...
    void testEnrichMovieTimeouts() {
        ...
        assertFalse(dto.getGenres().isEmpty());
        assertFalse(dto.getCountries().isEmpty());
        assertFalse(dto.getReviews().isEmpty());

        assertEquals("Втеча з Шоушенка", dto.getNameUkrainian());
        assertEquals("The Shawshank Redemption", dto.getNameNative());
        assertEquals(1994, dto.getYearOfRelease());
        assertEquals(123.45, dto.getPrice());
        assertEquals(9.5, dto.getRating());
        ...
        assertThat(dto.getReviews()).allSatisfy(r -> {
