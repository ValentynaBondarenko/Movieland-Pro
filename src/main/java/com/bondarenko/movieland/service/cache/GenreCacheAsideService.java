...
public class GenreCacheAsideService {
    ...
    private final CopyOnWriteArraySet<Genre> genreCache = new CopyOnWriteArraySet<>();
    ...
    private void cacheLoading() {
        Set<Genre> genresFromDb = fetchGenresFromDatabase();
        genreCache.addAll(genresFromDb);
        log.info("Genre cache updated with {} items", genreCache.size());
    }

    private Set<Genre> fetchGenresFromDatabase() {
